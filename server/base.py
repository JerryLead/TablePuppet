...
class Server:
    ...
    def train(self, epoch):
        if self.args.use_label_DP == True:
            self.rlp = RandomizedLabelPrivacy(self.args.label_DP_sigma)
            # Add noise to label
            if self.dataset.name == "MovieLens-1M":  # special handling
                self.soft_b = torch.stack([self._noise(t, 6) for t in self.b])
                self.b = torch.argmax(self.soft_b, dim=1).numpy()
            else:
                self.soft_b = torch.stack([self._noise(t, self.C) for t in self.b])
                self.b = torch.argmax(self.soft_b, dim=1).numpy()
            self.rlp.increase_budget()
            logger.info(
                f"Label privacy sigma = {self.args.label_DP_sigma}, eps = {self.rlp.privacy[0]}"
            )
        self.init()
        avg_exec_t = 0.0
        ...
        if self.task == "classification":
            logger.info(
                "[MaxAcc] max_train_acc = %.3f, max_test_acc = %.3f"
                % (max_train_acc, max_test_acc)
            )
        elif self.task == "regression":
            logger.info(
                "[MinRMSE] min_train_rmse = %.3f, min_test_rmse = %.3f"
                % (min_train_rmse, min_test_rmse)
            )
        if self.args.use_DP:
